...
function transformSVGIconImports(j, root) {
  ...
  root.find(j.Literal).filter(path => pathMatchRegex.test(path.node.value)).forEach(path => {
    const [, iconName] = path.node.value.match(pathMatchRegex); // update to new path
...
module.exports = function transformer(fileInfo, api) {
  ...
  const root = j(fileInfo.source); // transforms
