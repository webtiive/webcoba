...
function transformer(fileInfo, api, options) {
  ...
  root.find(j.ImportDeclaration).forEach(path => {
    ...
    let entryModule = importPath.match(importRegExp); // Remove non-MUI imports
    ...
    path.node.specifiers.forEach(specifier => {
      ...
      if (!importedName) {
        const importStatement = j.importDeclaration([j.importDefaultSpecifier(j.identifier(localName))], j.literal(`${targetModule}/${entryModule}`));
        j(path).insertBefore(importStatement);
      } else {
        const importStatement = j.importDeclaration([j.importDefaultSpecifier(j.identifier(localName))], j.literal(`${targetModule}/${importedName}`));
        j(path).insertBefore(importStatement);
      }
    });
    path.prune();
