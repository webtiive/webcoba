...
function transformer(file, api, options) {
  ...
  function getPrefix(withStylesCall) {
    let prefix; // 1. check from withStylesFn
  ...
  function getFirstJsxName() {
    ...
    if (matches) {
      const closingTag = matches.slice(-1)[0]; // Self closing tag
    ...
    if (name) {
      ...
      root.findJSXElements(name).at(0).forEach(path => {
        ...
        if (existingClassName) {
          if (existingClassName.value.type === 'StringLiteral') {// className="string"
          }

          if (existingClassName.value.type === 'JSXExpressionContainer') {
            if (existingClassName.value.expression.type === 'StringLiteral') {// className={'string'}
            }
  ...
  const withStylesCall = root.find(j.CallExpression, {
    callee: {
      name: 'withStyles'
    }
  }).nodes()[0];
  const makeStylesCall = root.find(j.CallExpression, {
    callee: {
      name: 'makeStyles'
    }
  }).nodes()[0];
